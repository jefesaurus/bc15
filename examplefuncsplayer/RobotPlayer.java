...
public class RobotPlayer {
    ...
    while (true) {
      ...
      if (rc.getType() == RobotType.BASHER) {
        try {
          ...
          // BASHERs attack automatically, so let's just move around mostly
          // randomly
          if (rc.isCoreReady()) {
  ...
  // This method will attempt to spawn in the given direction (or as close to it
  // as possible)
  static void trySpawn(Direction d, RobotType type) throws GameActionException {
    ...
    boolean blocked = false;
    while (offsetIndex < 8
        && !rc.canSpawn(directions[(dirint + offsets[offsetIndex] + 8) % 8],
            type)) {
      offsetIndex++;
    }
    if (offsetIndex < 8) {
  ...
  // This method will attempt to build in the given direction (or as close to it
  // as possible)

...
import launcherrambobot.MovingBot.AttackMode;
...
public class HQ extends BaseBot {
  ...
  public void execute() throws GameActionException {
    ...
    curNumAerospaceLabs = Messaging.checkTotalNumUnits(RobotType.AEROSPACELAB);
    ...
    curNumLaunchers = Messaging.checkTotalNumUnits(RobotType.LAUNCHER);
    ...
    */
    ...
    case BUILDING_FORCES:
      if (rc.getRoundLimit() - Clock.getRoundNum() <= 500 && !haveMoreTowers) {
  ...
  public static final int NUM_AEROSPACE_LABS = 10;
  public static final int NUM_HELIPADS = 1;
  public static int NUM_SUPPLY_DEPOTS = Math.max(4, teamOreValue / (9 * 90));
  ...
  public void doMacro() throws GameActionException {
    ...
    if (Messaging.peekQueueUnits(RobotType.LAUNCHER) < curNumAerospaceLabs) {
      Messaging.queueUnits(RobotType.LAUNCHER, curNumAerospaceLabs*2);
    }
    return;
  }
  ...
  public void doBuildOrder() throws GameActionException {
    ...
    if (curNumHelipads < NUM_HELIPADS && Messaging.peekBuildingUnits(RobotType.BARRACKS) >= 1) {
    ...
    if (curNumAerospaceLabs < NUM_AEROSPACE_LABS && Messaging.checkNumUnits(RobotType.HELIPAD) >= 1) {
      Messaging.queueUnits(RobotType.AEROSPACELAB, NUM_AEROSPACE_LABS - curNumAerospaceLabs);
    }
